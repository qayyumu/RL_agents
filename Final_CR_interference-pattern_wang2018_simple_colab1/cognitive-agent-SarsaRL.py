...
for epi in tqdm_e: #range(total_episodes):
    ...
    for time in range(max_env_steps):
        ...
        nextaction = epsilon_greedy_policy(next_state.argmax(),epsilon)
        
         # then we perform the action and move to the next state, and receive the reward
        #nextstate, reward, done, _ = env.step(action)
        
        # we calculate the Q value of previous state using our update rule
        q_table[(state.argmax(),action)] += alpha * (reward + gamma * q_table[(next_state.argmax(),nextaction)] - q_table[(state.argmax(),action)])
